...
it('can add the star name and star symbol properly', async() => {
    ...
    assert.strictEqual(symbol, 'AWD')
    assert.strictEqual(tokenName, 'Alien World')
});
...
it('lets 2 users exchange stars', async() => {
    ...
    let tokenId1 = 7;
    await instance.createStar('User Star 1!', tokenId1, {from: user1});

    let tokenId2 = 8;
    await instance.createStar('User Star 2!', tokenId2, {from: user2});
...
it('lets a user transfer a star', async() => {
    ...
    await instance.createStar('User Star!', tokenId, {from: fromUser});

    // 2. use the transferStar function implemented in the Smart Contract
    await instance.transferStar(toUser, tokenId);

    // 3. Verify the star owner changed.
    let star = await instance.ownerOf.call(tokenId);
    assert.equal(star,toUser);
});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let tokenId = 10;
    let instance = await StarNotary.deployed();
    await instance.createStar('Wow Star!', tokenId, {from: accounts[0]});

    // 2. Call your method lookUptokenIdToStarInfo
    let name = await instance.lookUptokenIdToStarInfo.call(tokenId);

    // 3. Verify if you Star name is the same
    assert.equal(name, 'Wow Star!')    
});